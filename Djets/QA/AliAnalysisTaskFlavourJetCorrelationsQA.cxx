...
ClassImp(AliAnalysisTaskFlavourJetCorrelationsQA)


//_______________________________________________________________________________

AliAnalysisTaskFlavourJetCorrelationsQA::AliAnalysisTaskFlavourJetCorrelationsQA() :
...
AliAnalysisTaskFlavourJetCorrelationsQA::AliAnalysisTaskFlavourJetCorrelationsQA(const Char_t* name, AliRDHFCuts* cuts,ECandidateType candtype) :
...
void AliAnalysisTaskFlavourJetCorrelationsQA::Init(){
   //
   // Initialization
   //
...
void AliAnalysisTaskFlavourJetCorrelationsQA::UserCreateOutputObjects() {
   // output
...
Bool_t AliAnalysisTaskFlavourJetCorrelationsQA::Run()
{
...
if(fUseMCInfo && fBuildRMEff){
    ...
    while ((jet = mcjets->GetNextJet()))
    {
        ...
        fhPhiJetPtJet->Fill(jet->Pt(),jet->Phi());

        fhAreaJetPtJet->Fill(jet->Pt(),jet->Area());


        Int_t ntrjet=  jet->GetNumberOfTracks();
        fhJetTrksPtJet->Fill(jet->Pt(),ntrjet);
        ...
        for(Int_t itrk=0;itrk<ntrjet;itrk++)
        {
            ...
            fhPhiJetTrksPtJet->Fill(jet->Pt(),jetTrk->Phi());
...
void AliAnalysisTaskFlavourJetCorrelationsQA::ConstituentCorrelationMethod(Bool_t IsBkg, AliAODEvent* aodEvent)
{
...
void AliAnalysisTaskFlavourJetCorrelationsQA::AngularCorrelationMethod(Bool_t IsBkg, AliAODEvent* aodEvent)
{
    AliJetContainer* JetCont = GetJetContainer(0);
...
void AliAnalysisTaskFlavourJetCorrelationsQA::CreateResponseMatrix(AliEmcalJet* jet)
{
...
void AliAnalysisTaskFlavourJetCorrelationsQA::CreateMCResponseMatrix(AliEmcalJet* MCjet, AliAODEvent* aodEvent)
{
    ...
    Double_t JetNTracksGen = MCjet->GetNumberOfTracks();
    ...
    if(JetContRec){
        ...
        if (jet){
            ...
            JetEtaRec = jet->Eta();
            JetPhiRec = jet->Phi();
            JetNTracksRec = jet->GetNumberOfTracks();
            DPtRec = Drec->Pt();
            ...
            {
                AliAODRecoDecayHF* dzero=(AliAODRecoDecayHF*)Drec;
                FillHistogramsD0JetCorr(dzero,zRec,Drec->Pt(),JetPtRec,JetEtaRec,kFALSE,bDInEMCalAcc,bJetInEMCalAcc,aodEvent,pdg,JetPhiGen,JetNTracksGen,JetPhiRec,JetNTracksRec);
            }
            else if(fCandidateType==kDstartoKpipi)
            {
                AliAODRecoCascadeHF* dstar=(AliAODRecoCascadeHF*)Drec;
...
void AliAnalysisTaskFlavourJetCorrelationsQA::FillDJetHistograms(AliEmcalJet* jet, Double_t rho, Bool_t IsBkg, AliAODEvent* aodEvent)
{
    ...
    if(fCandidateType==kD0toKpi)
    {
        ...
        FillHistogramsD0JetCorr(dzero,z,Dmeson->Pt(),JetPtCorr,JetEtaRec,IsBkg,bDInEMCalAcc,bJetInEMCalAcc,aodEvent,-999,-999,-999,-999,-999);
    }
    else if(fCandidateType==kDstartoKpipi)
    {
...
void AliAnalysisTaskFlavourJetCorrelationsQA::GetHFJet(AliEmcalJet*& jet, Bool_t IsBkg)
{
    AliJetContainer* JetCont = 0;
...
void AliAnalysisTaskFlavourJetCorrelationsQA::FindMCJet(AliEmcalJet*& mcjet)
{
...
void AliAnalysisTaskFlavourJetCorrelationsQA::Terminate(Option_t*)
{
...
void  AliAnalysisTaskFlavourJetCorrelationsQA::SetMassLimits(Double_t range, Int_t pdg){
   Float_t mass=0;
...
void  AliAnalysisTaskFlavourJetCorrelationsQA::SetMassLimits(Double_t lowlimit, Double_t uplimit){
   if(uplimit>lowlimit)
   {
      fMinMass = lowlimit;
      fMaxMass = uplimit;
...
Bool_t AliAnalysisTaskFlavourJetCorrelationsQA::SetD0WidthForDStar(Int_t nptbins,Float_t *width){
   if(nptbins>30) {
...
Double_t AliAnalysisTaskFlavourJetCorrelationsQA::Z(AliVParticle* part,AliEmcalJet* jet, Double_t rho) const{
...
Double_t AliAnalysisTaskFlavourJetCorrelationsQA::Z(AliVParticle* part,AliEmcalJet* jet) const{
...
//_______________________________________________________________________________
Double_t AliAnalysisTaskFlavourJetCorrelationsQA::Z(Double_t* p, Double_t *pj) const{
...
//_______________________________________________________________________________
Double_t AliAnalysisTaskFlavourJetCorrelationsQA::ZT(Double_t* p, Double_t *pj) const{
...
Bool_t  AliAnalysisTaskFlavourJetCorrelationsQA::DefineHistoForAnalysis(){
   ...
   fhPhiJetPtJet  = new TH2F("fhPhiJetPtJet","Jet #phi distribution vs Jet p_{T}; Jet p_{T};#phi (rad)",nbinsptjet,ptjetlims[0],ptjetlims[1], nbinsphi,philims[0],philims[1]);
   fhPhiJetTrksPtJet = new TH2F("fhPhiJetTrksPtJet","Jet tracks #phi distribution vs Jet p_{T}; Jet p_{T};#phi (rad)",nbinsptjet,ptjetlims[0],ptjetlims[1], nbinsphi,philims[0],philims[1]);
   fhAreaJetPtJet= new TH2F("fhAreaJetPtJet","Jet area distribution vs Jet p_{T}; Jet p_{T};Area",nbinsptjet,ptjetlims[0],ptjetlims[1], 10,0,1);
   fhJetTrksPtJet= new TH2F("fhJetTrksPtJet","Jet N tracks distribution vs Jet p_{T}; Jet p_{T};N tracks",nbinsptjet,ptjetlims[0],ptjetlims[1], 50,0,50);
   ...
   fhJetTrksPtJetD= new TH2F("fhJetTrksPtJetD","D Jet N tracks distribution vs Jet p_{T}; Jet p_{T};N tracks",nbinsptjet,ptjetlims[0],ptjetlims[1], 50,0,50);

   fhPhiJetPtJet->Sumw2();
   fhPhiJetTrksPtJet->Sumw2();
   fhAreaJetPtJet->Sumw2();
   fhJetTrksPtJet->Sumw2();
   ...
   fOutput->Add(fhPhiJetPtJet);
   fOutput->Add(fhPhiJetTrksPtJet);
   fOutput->Add(fhAreaJetPtJet);
   fOutput->Add(fhJetTrksPtJet);
   ...
   fOutput->Add(fhJetTrksPtJetD);
    ...
    if(fUseMCInfo){
        if(fCandidateType==kDstartoKpipi){
          ...
          const Int_t nbinsSparse[nAxis]={nbinsSpsz,nbinsSpsptjet,nbinsSpsptD,nbinsSpsmass,nbinsSpsy,nbinsSpsy,nbinsSpsmass,nbinsSpsmass, 2, 2, 2,nbinsphi,50,nbinsphi,50};
          const Double_t minSparse[nAxis]={zlims[0],ptjetlims[0],ptDlims[0],fMinMass,etalims[0],etalims[0],fMinMass,fMinMass, -0.5,-0.5,-0.5,philims[0],0,philims[0],0};
          const Double_t maxSparse[nAxis]={zlims[1],ptjetlims[1],ptDlims[1],fMaxMass,etalims[1],etalims[1],fMaxMass,fMaxMass, 1.5, 1.5 , 1.5,philims[1],50,philims[1],50};
          fNAxesBigSparse=nAxis;
          fhsDphiz=new THnSparseF("hsDphiz","Z, p_{T}^{jet}, p_{T}^{D}, mass., y^{D}, #eta^{jet}, mass_{true}, mass_{refl}, Bkg?, D in EMCal acc?, jet in EMCal acc?, Jet #Phi gen, Jet N tracks gen, Jet #Phi rec, Jet N tracks rec", nAxis, nbinsSparse, minSparse, maxSparse);
        }
...
void AliAnalysisTaskFlavourJetCorrelationsQA::FillHistogramsD0JetCorr(AliAODRecoDecayHF* candidate, Double_t z, Double_t ptD, Double_t ptj, Double_t jetEta, Bool_t IsBkg, Bool_t bDInEMCalAcc, Bool_t bJetInEMCalAcc, AliAODEvent* aodEvent, Int_t pdgTrue,Double_t JetPhiGen,Double_t JetNTracksGen,Double_t JetPhiRec,Double_t JetNTracksRec){
   ...
   else
   {
      ...
      point[11]=JetPhiGen;
      point[12]=JetNTracksGen;
      point[13]=JetPhiRec;
      point[14]=JetNTracksRec;
...
void AliAnalysisTaskFlavourJetCorrelationsQA::FillHistogramsDstarJetCorr(AliAODRecoCascadeHF* dstar,  Double_t z, Double_t ptD, Double_t ptj, Double_t jetEta, Bool_t IsBkg, Bool_t bDInEMCalAcc, Bool_t bJetInEMCalAcc){
...
void AliAnalysisTaskFlavourJetCorrelationsQA::FillHistogramsMCGenDJetCorr(Double_t z,Double_t ptD,Double_t ptjet, Double_t yD, Double_t jetEta, Bool_t bDInEMCalAcc, Bool_t bJetInEMCalAcc){
...
Float_t AliAnalysisTaskFlavourJetCorrelationsQA::DeltaR(AliEmcalJet *p1, AliVParticle *p2, Double_t rho) const {
...
//_______________________________________________________________________________
Float_t AliAnalysisTaskFlavourJetCorrelationsQA::CheckDeltaR(AliEmcalJet *p1, AliVParticle *p2) const {
    //Calculate DeltaR between p1 and p2: DeltaR=sqrt(Delataphi^2+DeltaEta^2)
    if(!p1 || !p2) return -1;
...
Int_t AliAnalysisTaskFlavourJetCorrelationsQA::IsDzeroSideBand(AliAODRecoCascadeHF *candDstar){
...
Bool_t AliAnalysisTaskFlavourJetCorrelationsQA::InEMCalAcceptance(AliVParticle *vpart){
