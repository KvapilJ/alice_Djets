...
void drawFDZ()
{
  ...
  // TFile *inFileFD1 = new TFile(Form("/home/kvapil/work/analysis/pp_run2/D0jet/BaseCuts_z04F/Default_AnalysisResults_Run2w18b.root/FDsubtraction2D/JetPtSpectrum_FDsub%d.root",z2),"read");
...
std::cout<<"A"<<std::endl;
          ...
          //  hFD_ratio1->SetMaximum(hFD_ratio1->GetMaximum()*3);
            ...
            hFD_ratio_err1->GetYaxis()->SetTitle("Entries");
...
hFD_data->GetXaxis()->SetTitle("#it{z}^{ch}_{||}");
...
TLegend *leg = new TLegend(0.3,0.62,0.7,0.78);
...
std::cout<<"A"<<std::endl;
    TPaveText *pt = new TPaveText(0.12,0.82,0.8,0.95,"NB NDC");
    pt->SetBorderSize(0);
    pt->SetFillStyle(0);
    pt->SetTextAlign(13);
    pt->SetTextFont(42);
    ...
    text = pt->AddText(tmp+"pp, #sqrt{#it{s}} = 13 TeV, charged jets with D^{0}");
    text = pt->AddText(tmp+"anti-#it{k}_{T}"+Form(" #it{R} = 0.%d, |#it{#eta}_{lab}^{jet}| < 0.%d",4,5));

    TPaveText *pt2 = new TPaveText(0.65,0.81,0.8,0.95,"NB NDC");
    pt2->SetBorderSize(0);
    pt2->SetFillStyle(0);
    pt2->SetTextAlign(13);
    pt2->SetTextFont(42);
    ...
    if(z1==2)text2 = pt2->AddText(Form("#splitline{%d < #it{p}_{T,D^{0}} < %d GeV/#it{c}}{%d < #it{p}_{T,jet} < %d GeV/#it{c}}",2,7,5,7));
    if(z1==5)text2 = pt2->AddText(Form("#splitline{%d < #it{p}_{T,D^{0}} < %d GeV/#it{c}}{%d < #it{p}_{T,jet} < %d GeV/#it{c}}",5,36,15,50));
    ...
    cEff->cd()->SetLogy();
    hFD_ratio2->GetYaxis()->SetRangeUser(500,1E6);
    if(z1==5)hFD_ratio2->GetYaxis()->SetRangeUser(10,6E5);
...
std::cout<<"A"<<std::endl;
 ...
 //   cEff->SaveAs(Form("DjetFD_sum.png"));
    cEff->SaveAs(Form("DjetFD_z%d_thesis.pdf",z1));
