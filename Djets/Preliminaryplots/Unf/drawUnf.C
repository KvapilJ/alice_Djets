...
void drawUnf()
{
   ...
   TFile *inFileFD1 = new TFile("/home/kvapil/work/analysis/pp_run2/D0jet/BaseCuts_X04/Default_AnalysisResults_Run2w18b.root/unfolding_Bayes_5/unfoldedSpectrum_unfoldedJetSpectrum.root","read");
...
text = pt->AddText(Form ("with D^{0}, %d < #it{p}_{T,D^{0}} < %d GeV/#it{c}",2,36));
 ...
 /*   TCanvas *cEff = new TCanvas("cEff","cEff",1000,800);
   ...
   /* hFD_ratio_err2->SetFillColor(1);
    ...
    TH1D *hData_binned_ratio2 = dynamic_cast<TH1D*>(hFD_data->Clone("hFD_ratio1_ration2"));
    hData_binned_ratio2->Divide(hFD_data,hFD_ratio2,1,1,"B");
    upPad->cd();
...
std::cout<<"A"<<std::endl;
  //  leg->Draw("same");
    ...
    canvas->SaveAs(Form("DjetUnf_thesis.pdf"));
