...
#include "/home/kvapil/alice/sw/SOURCES/RooUnfold/V02-00-01-alice4/V02-00-01-alice4/examples/RooUnfoldTestHarness2D.h"
...
#include "/home/kvapil/alice/sw/SOURCES/RooUnfold/V02-00-01-alice4/V02-00-01-alice4/examples/ArgVars.h"
...
void RooUnfoldTestHarness2D::Parms (ArgVars& args)
{
  ...
  args.Add ("fakeylo", &fakeylo,    0.0, "fraction of fakes at ylo");
  args.Add ("fakeyhi", &fakeyhi,    0.0, "fraction of fakes at yhi");
...
{
  if (mscalex!=1.0) {
    hTrainX->Scale(mscalex);
    if (hTrainFakeX) hTrainFakeX->Scale(mscalex);
  }
  if (mscaley!=1.0) {
    hTrainY->Scale(mscaley);
    if (hTrainFakeY) hTrainFakeY->Scale(mscaley);
  }
...
{
  if (mscalex!=1.0) {
    hMeasX->Scale(mscalex);
    if (hFakeX) hFakeX->Scale(mscalex);
  }
  if (mscaley!=1.0) {
    hMeasY->Scale(mscaley);
    if (hFakeY) hFakeY->Scale(mscaley);
  }
