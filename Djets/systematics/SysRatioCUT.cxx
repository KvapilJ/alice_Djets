...
void SysRatio::GetRMS(){
    ...
    for(int bin=1; bin<=TH1DDefault_->GetNbinsX(); bin++){
        Double_t rms_bin = 0;
        for (UInt_t file=0; file<nFiles_+1; file++){
...
void SysRatio::Load(){
    ...
    //TFile *ftmp = new ("/home/kvapil/work/analysis/pp_run2/D0jet/BaseCuts_PRELIM/Cuts_VarDefT2f/Default/unfolding_2D_5/","read")
    ...
    for(UInt_t f = 0; f < nFiles_; f++){
        ...
        legRatio_->AddEntry(hRatio_[f],legendVarDesc_[f],"lp");
        TString opt{"hist"};
...
void SysRatio::Draw(){
    ...
    if(hstatUp_){
        hStackRatio_->Add(hstatUp_,"hist");
        legRatio_->AddEntry(hstatUp_,"Stat. unc.","l");
    }
    if(hstatDown_){
        hStackRatio_->Add(hstatDown_,"hist");
    }
   ...
   // hStackMain_->GetYaxis()->SetTitle("dN/dz_{#parallel}");
    hStackMain_->GetYaxis()->SetTitle("efficiency");
    //hStackMain_->GetXaxis()->SetTitle("z_{#parallel}");
    hStackMain_->GetXaxis()->SetTitle("p_{T,D}");
    ...
    hStackRatio_->SetMaximum(maxR*(1+0.7*(maxR-1)/maxR)); //2.1*m-1.1
    //hStackRatio_->GetXaxis()->SetTitle("z_{#parallel}");
    hStackRatio_->GetXaxis()->SetTitle("p_{T,D}");
