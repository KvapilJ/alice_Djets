...
//getAvg(nFiles,hratio,hmean,hsys);
            ...
            for (int i=0; i<nFiles; i++){
                dirName[i] = inDirBase;
                dirName[i] += "/";
                dirName[i] += "Default_AnalysisResults_Run2.root/";
                dirName[i] += inDir[i];
        ...
        //    return;
          ...
          cspec->SaveAs(Form("%s/UnfoldingRangesComparison.png",out.Data()));
            ...
            for(int i=0; i<nFiles-1; i++){
                ...
                hratio[i]->GetYaxis()->SetRangeUser(0.98,1.02);
            ...
            hsys->GetYaxis()->SetRangeUser(0,0.3);
  ...
  for(int i=0; i<fptbinsJetFinalN; i++){
      ...
      for (int j=0; j<nFiles-1; j++){
          mean[i] = mean[i]+ ( hratio[j]->GetBinContent(
							hratio[j]->FindBin( 
                                    (fptbinsJetFinalA[i]+fptbinsJetFinalA[i+1])/2.
									)
							) 
				);
